	  ...
	  <div id="corpocad">
		...
		<?php 
			if (isset($_POST['enviar'])) {
				...
				if (!empty($titulo)){
					$sql = "DELETE FROM `tarefa` WHERE `tarefa`.`titulo` = '$titulo'";

					// executa a consulta
					if (mysqli_query($conn, $sql) ){
						// consulta executada com sucesso
						// exibindo um poupup javascript para informar o usuario
						echo "<script>alert('Tarefa Removida com sucesso!');</script>";
					} else {
						// houve um erro ao executar a consulta
						echo "<br>O titulo informado não está no banco de dados. Verifique o titulo e tente novamente.<br>";
					}
